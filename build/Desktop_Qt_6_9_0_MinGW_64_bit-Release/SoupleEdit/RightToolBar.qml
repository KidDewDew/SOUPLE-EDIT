    ...
    Component {
        ...
        ToolBar_01 {
            items: [
                ...
                {
                    name: "表格转Excel",
                    img_src: "qrc:/image/icon_table2excel.png",
                    onClicked: function() {
                        if (Helper.temp_Get("TableToExcelTool_opened", false) === false) {
                            // 用Component.createObject替代，明确指定文件路径
                            var excelToolComponent = Qt.createComponent("qrc:/qt/qml/SoupleEdit/Dialog_TableToExcelTool.qml");
                            if (excelToolComponent.status === Component.Ready) {
                                excelToolComponent.createObject(window, { visible: true });
                            } else {
                                console.error("表格转Excel组件加载失败：", excelToolComponent.errorString());
                            }
                        } else {
                            messageBox("提示", "表格转Excel工具已打开");
                        }
                    }
                }
                ...
                /*{
                    ...
                    onClicked: function() {
                        if (Helper.temp_Get("TableToExcelTool_opened", false) === false) {
                                    ...
                                    Qt.createQmlObject(`
                                        import QtQuick
                                        Dialog_TableToExcelTool { visible: true; }
                                    `, window);

                        } else {
                            messageBox("Info", "表格转Excel工具已打开");
                        }
                    }
                }*/
            ]
